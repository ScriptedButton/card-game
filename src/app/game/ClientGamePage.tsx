...
export default function ClientGamePage({
  ...
  useEffect(() => {
    ...
    const handleBalanceUpdate = (event: CustomEvent) => {
      if (event.detail && typeof event.detail.balance === "number") {
        // Update any UI elements that need to reflect the balance
        console.log(`Balance update received: ${event.detail.balance}`);
      }
    };
  ...
  useEffect(() => {
    ...
    // For guest users, store a flag in localStorage
    if (isGuest) {
  ...
  }, [isGuest, initialPlayerName, setPlayerName]);
  ...
  // Show a badge for guest users
  const renderGuestBadge = () => {
    if (!isGuest) return null;

    return (
      <div className="absolute top-4 left-4 z-50 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/20 shadow-lg">
  ...
  return (
    <div
      className="relative min-h-screen game-table flex flex-col"
    ...
    >
        ...
        <div className="max-w-7xl mx-auto px-4 py-2 flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <span className="text-white/80">
              {initialPlayerName ? `Player: ${initialPlayerName}` : "Welcome!"}
            </span>
      ...
      {/* Game content - Add pt-16 to account for fixed header */}
      <main className="flex-1 flex flex-col relative z-10 pt-16 overflow-y-auto">
        {/* Dealer Area */}
        <div className="dealer-area min-h-[250px] w-full max-w-7xl mx-auto px-4 relative pt-4">
          <div className="absolute top-5 left-4 text-yellow-300/80 font-semibold bg-black/30 backdrop-blur-sm px-3 py-1 rounded-full text-sm">
            Dealer
          </div>
        ...
        {/* Player Area */}
        <div className="player-area min-h-[300px] w-full max-w-7xl mx-auto px-4 pb-6 relative mb-6">
      ...
      {/* Game Info Footer - Changed to static positioning with proper margin */}
      <div className="bg-black/40 backdrop-blur-sm p-2 flex justify-center items-center mt-auto">
